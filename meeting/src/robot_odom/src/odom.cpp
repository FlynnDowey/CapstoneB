...
int main(int argc, char** argv)
{
    ...
    double encoder_resolution = 1440; // encoder counts per revolution
  ...
  */
        ...
        double wheel1_velocity = -((encoder0Pos - old_encoder0Pos) / encoder_resolution) * (2 * 3.14 * wheel_radius) / dt;
        double wheel2_velocity = -((encoder1Pos - old_encoder1Pos) / encoder_resolution) * (2 * 3.14 * wheel_radius) / dt;
        double wheel3_velocity = -((encoder2Pos - old_encoder2Pos) / encoder_resolution) * (2 * 3.14 * wheel_radius) / dt;
        double wheel4_velocity = -((encoder3Pos - old_encoder3Pos) / encoder_resolution) * (2 * 3.14 * wheel_radius) / dt;
        ...
        // Assuming a holonomic drive robot (Mecanum or omni-directional), calculate linear and angular velocities
        // Assuming a holonomic drive robot (Mecanum or omni-directional), calculate linear and angular velocities
        ...
        vth = (wheel1_velocity + wheel2_velocity - wheel3_velocity - wheel4_velocity) / (4.0 * (wheelbase / 2 + track / 2)); // Z-axis angular velocity
